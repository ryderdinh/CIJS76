...
searchBox.addEventListener('submit', (e) => {
  ...
  const value = searchBox.elements['search-input'].value;

  const result = filterPerson(value);

  let resultHTML = '';

  result.forEach((person) => {
    for (let key in person) {
      resultHTML += `<p>${key}: ${person[key]}</p>`;
    }
    resultHTML += `<br/><br/>`;
  });
...
const filterPerson = (value) =>
  persons.filter((person) => person.name.includes(value));
